    ...
    onAdd: Task.async (function* () {
        if (document.documentElement.getButton("extra1").disabled == false) {
            ...
            if (servertype == "auto") {
                ...
                //do what ever you need to do for autodiscover here, sleep just simulates server activity
                //let result = yield tbSync.ews.getServerConnectionViaAutodiscover(user, password, tbSyncEwsNewAccount.maxTimeout*1000);
                yield tbSync.sleep(10000, false);
                let result = {"server": "https://www.test.de", "user": user, "errorcode": "200", "error": ""};
