...
var tbSyncAccountSettings = {
    ...
    */
    loadSettings: function () {
        let settings = tbSync.ews.getAccountStorageFields();
        let servertype = tbSync.db.getAccountSetting(tbSyncAccountSettings.selectedAccount, "servertype");
        let fixedSettings = tbSync.ews.getFixedServerSettings(servertype);
        ...
        document.getElementById("tbsync.accountsettings.config.label").value= tbSync.getLocalizedMessage("config." + servertype, "ews");
    ...
    unlockSettings: function () {
        if (confirm(tbSync.getLocalizedMessage("prompt.UnlockSettings", "ews"))) {
    ...
    updateSyncstate: function () {        
        ...
        if (isSyncing) {
            ...
            let msg = tbSync.getLocalizedMessage("syncstate." + syncstate, "ews");
            if (diff > 2000) msg = msg + " (" + Math.round((tbSync.prefSettings.getIntPref("ews.timeout") - diff)/1000) + "s)";
            ...
            if (isConnected) document.getElementById('tbsync.accountsettings.syncbtn').label = tbSync.getLocalizedMessage("status.syncing", "ews");
            else document.getElementById('tbsync.accountsettings.syncbtn').label = tbSync.getLocalizedMessage("status.connecting", "ews");            

            //do not display slider while syncing
            document.getElementById('tbsync.accountsettings.slider').hidden = true;
            
        } else {
            let localized = tbSync.getLocalizedMessage("status." + status, "ews");
            if (!isEnabled) localized = tbSync.getLocalizedMessage("status." + "disabled", "ews");
            ...
            if (isConnected) document.getElementById('tbsync.accountsettings.syncbtn').label = tbSync.getLocalizedMessage("button.syncthis", "ews");            
            else document.getElementById('tbsync.accountsettings.syncbtn').label = tbSync.getLocalizedMessage("button.tryagain", "ews");            
        ...
        if (isEnabled) document.getElementById('tbsync.accountsettings.enablebtn').label = tbSync.getLocalizedMessage("button.disableAndEdit", "ews");
        else document.getElementById('tbsync.accountsettings.enablebtn').label = tbSync.getLocalizedMessage("button.enableAndConnect", "ews");
    },
    ...
    toggleFolder: function () {
        ...
        if (folderList.selectedItem !== null && !folderList.disabled) {
            ...
            if (folder.selected == "1") {
                if (window.confirm(tbSync.getLocalizedMessage("prompt.Unsubscribe", "ews"))) {
                    //deselect folder
                    folder.selected = "0";
                    //remove folder, which will trigger the listener in tbsync which will clean up everything
                    tbSync.ews.removeTarget(folder.target, folder.type); 
                }
            } else {
    ...
    updateMenuPopup: function () {
        ...
        if (!folderList.disabled && folderList.selectedItem !== null && folderList.selectedItem.value !== undefined) {
            ...
            if (folder.selected == "1") {
                document.getElementById("tbsync.accountsettings.ContextMenuToggleSubscription").label = tbSync.getLocalizedMessage("subscribe.off::" + folder.name, "ews");
            } else {
                document.getElementById("tbsync.accountsettings.ContextMenuToggleSubscription").label = tbSync.getLocalizedMessage("subscribe.on::" + folder.name, "ews");
            }
	}
                    ...
                    switch (status) {
                        ...
                        case "modified":
                            if (type == "7" || type == "15" || type == "8" || type == "13") {
                                if (tbSync.lightningIsAvailable()) status = tbSync.getLocalizedMessage("status." + status, "ews") + ": "+ tbSync.getCalendarName(folders[folderIDs[i]].target);
                                else status = tbSync.getLocalizedMessage("status.nolightning", "ews");
                            }
                            if (type == "9" || type == "14") status = tbSync.getLocalizedMessage("status." + status, "ews") + ": "+ tbSync.getAddressBookName(folders[folderIDs[i]].target);
                            break;
                        case "pending":
                            let syncdata = tbSync.getSyncData(tbSyncAccountSettings.selectedAccount);
                            status = tbSync.getLocalizedMessage("status." + status, "ews");
                            if (folderIDs[i] == syncdata.folderID) {
                                status = tbSync.getLocalizedMessage("status.syncing", "ews");
                ...
                if (syncstate == "accountdone") {
                        ...
                        switch (status) {
                            ...
                            case "disabled":
                                ...
                                break;
                            default:
                                msg = tbSync.getLocalizedMessage("status." + status, "ews");
                        }
                }
