...
calExchangeCalendar.prototype = {
    ...
    masterModified: function _masterModified(aModifiedMaster) {
        ...
        for (let item of this.itemCacheById) {
    ...
    getExceptions: function _getExceptions(aRecurrenceItems) {
        var tmpCount = 0;
        var exceptions = {};
        for (let recurrenceItem of aRecurrenceItems) {
            tmpCount++;
            ...
            if (recurrenceItem.isNegative) {
                ...
                let occurrences = recurrenceItem.getOccurrences(this.startDate, this.startDate, this.endDate, 0, {});
    ...
    getRemovedOccurrence: function _getRemovedOccurrence(aOldItem, aNewItem) {
        ...
        if (newCount.value > oldCount.value) {
            ...
            for (let newOccurrence of newOccurrences) {
                var foundOld = false;
                for (let oldOccurrence of oldOccurrences) {
        ...
        if (newCount.value > oldCount.value) {
            ...
            for (let newException of newExceptions) {
                var foundOld = false;
                for (let oldException of oldExceptions) {
    ...
    syncInboxOK: function _syncInboxOK(erSyncInboxRequest, creations, updates, deletions, syncState) {
        ...
        if (creations.meetingrequests.length > 0) {
            for (let request of creations.meetingrequests) {
        ...
        if (updates.meetingrequests.length > 0) {
            for (let update of updates.meetingrequests) {
        ...
        if (deletions.meetingrequests.length > 0) {
            for (let deletion of deletions.meetingrequests) {
        ...
        if (creations.meetingCancellations.length > 0) {
            for (let request of creations.meetingCancellations) {
        ...
        if (updates.meetingCancellations.length > 0) {
            for (let update of updates.meetingCancellations) {
        ...
        if (deletions.meetingCancellations.length > 0) {
            for (let deletion of deletions.meetingCancellations) {
        ...
        for (let index of this.meetingRequestsCache) {
            if (index) {
        ...
        for (let index of this.meetingCancelationsCache) {
            if (index) {
        ...
        for (let response of creations.meetingResponses) {
            this.meetingResponsesCache[xml2json.getAttributeByTag(response, "t:ItemId", "Id")] = response;
        }

        for (let response of updates.meetingResponses) {
        ...
        for (let response of deletions.meetingResponses) {
            if (this.meetingResponsesCache[xml2json.getAttributeByTag(response, "t:ItemId", "Id")]) {
        ...
        if (this.doAutoRemoveInvitationResponse1) {
            for (let response of this.meetingResponsesCache) {
    ...
    removeChildrenFromMaster: function _removeChildrenFromMaster(aMaster) {
        ...
        for (let child of aMaster.getExceptions({})) {
        ...
        for (let child of aMaster.getOccurrences({})) {
            this.notifyTheObservers("onDeleteItem", [child]);
        ...
        if (!isMeetingRequest) {
            ...
            case "RecurringMaster":
                ...
                if ((this.recurringMasterCache[item.uid]) && (this.recurringMasterCache[item.uid].changeKey != item.changeKey)) {
                    ...
                    for (let tmpException of myExceptions) {
                        ids.push({
                            Id: tmpException.id,
                            type: "Exception",
                            uid: item.uid,
                    ...
                    for (let tmpOccurrence of myOccurrences) {
                        ids.push({
                            Id: tmpOccurrence.id,
                            type: "Occurrence",
                            uid: item.uid,
